...
const InfoCard: React.FC<{ icon: React.ReactElement; title: string; children: React.ReactNode; delay: number; }> = ({ icon, title, children, delay }) => (
    <div className="bg-white p-3 rounded-xl border border-zinc-200 shadow-sm opacity-0 animate-fadeInUp" style={{ animationDelay: `${delay}ms`, animationFillMode: 'forwards' }}>
...
const InfoItem: React.FC<{ label: string; children: React.ReactNode; }> = ({ label, children }) => (
    <div className="border-b border-zinc-100 pb-1.5 last:border-b-0 last:pb-0">
        <p className="font-semibold text-zinc-500 text-xs">{label}</p>
...
const InfoPage: React.FC<{ onLogout: () => void; event: Event | null }> = ({ onLogout, event }) => {
    ...
    return (
        <div className="pb-4 animate-fadeIn">
            <header className="sticky top-0 md:static bg-white md:bg-transparent z-10 p-3 md:pt-4 border-b border-zinc-200 flex justify-between items-center">
            ...
            <main className="p-3 space-y-3 md:grid md:grid-cols-2 md:gap-4 md:space-y-0">
                ...
                <InfoCard icon={IconDollar} title="Valores e Pagamento" delay={200}>
                    ...
                    <InfoItem label="Formas de Pagamento"><p>Cartão de Débito/Crédito e PIX.</p><p className="mt-1 p-1.5 bg-zinc-100 rounded-md text-center font-mono text-xs">Chave Pix: <Highlight>{event.pix_key}</Highlight></p></InfoItem>
                ...
                <div className="text-center text-sm text-zinc-600 bg-green-50 p-3 rounded-xl border border-green-200 opacity-0 animate-fadeInUp md:col-span-2" style={{ animationDelay: '350ms', animationFillMode: 'forwards' }}>
                    ...
                    <p className="font-bold mt-2 text-green-700">Axé!</p>
                </div>
            </main>
        </div>
    );
